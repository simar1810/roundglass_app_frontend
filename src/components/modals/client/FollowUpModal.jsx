...
function FollowUpModalContainer({ clientId }) {
  const { stage } = useCurrentStateContext();
  if (stage === 1) return <Stage1 />
  return <Stage2 clientId={clientId} />
}
...
function Stage2({ clientId }) {
  ...
  const healthMatrices = {
    ...
    muscle: calculateSkeletalMassPercentage({ ...healthMatrix, bodyComposition: healthMatrix.body_composition }),
    fat: calculateBodyFatPercentage({ ...healthMatrix, bodyComposition: healthMatrix.body_composition }),
    rm: calculateBMR({ ...healthMatrix, bodyComposition: healthMatrix.body_composition }),
    ideal_weight: (21 * (heightinMetres * heightinMetres)).toFixed(2),
    bodyAge: calculateBodyAge({ ...healthMatrix, bodyComposition: healthMatrix.body_composition }),
  }
  ...
  return <div className="p-4">
    <div className="grid grid-cols-3 gap-6">
      {matrices.filter(matrix => Boolean(healthMatrices[matrix.name])).map(matrix => <div
        key={matrix.id}
        className="bg-[var(--comp-1)] p-4 rounded-[8px]"
      >
...
function SelectBodyComposition() {
  ...
  return <div className="mt-4 col-span-2">
    ...
    <div className="grid grid-cols-5 gap-2">
      <div
        ...
        className={`border rounded p-3 text-center cursor-pointer w-24 ${healthMatrix.body_composition === "Slim" && "border-[var(--accent-1)]"}`}
      >
      ...
      <div
        onClick={() => dispatch(changeFieldvalue("body_composition", "Medium"))}
        className={`border rounded p-3 text-center cursor-pointer w-24 ${healthMatrix.body_composition === "Medium" && "border-[var(--accent-1)]"}`}
      >
        <div className="w-[83px] h-[106px] mx-auto mb-1 flex items-center justify-center overflow-hidden">
          <Image
            src="/svgs/medium.svg"
      ...
      <div
        ...
        className={`border rounded p-3 text-center cursor-pointer w-24 ${healthMatrix.body_composition === "Fat" && "border-[var(--accent-1)]"}`}
      >
        <div className="w-[83px] h-[106px] mx-auto mb-1 flex items-center justify-center overflow-hidden">
          <Image
            src="/svgs/fat.svg"
