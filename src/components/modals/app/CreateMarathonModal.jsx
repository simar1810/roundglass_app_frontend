...
import { newMarathonInitialState } from "@/config/state-data/new-marathon";
import { addTask, changeField, changeSubmissionReuirements, deleteTask, generatePayload, newMarathonReducer, selectTask } from "@/config/state-reducers/new-marathon";
...
export default function CreateMarathonModal() {
  return <Dialog>
    <DialogTrigger className="bg-[var(--accent-1)] text-[var(--primary-1)] text-[14px] font-semibold px-3 py-2 rounded-[8px]">
      Add
    </DialogTrigger>
...
function MarathonContainer() {
  ...
  async function createMaraton() {
    try {
      ...
      const response = await sendData("app/marathon/coach/createMarathon", data);
