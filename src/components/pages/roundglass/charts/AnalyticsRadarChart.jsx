...
function transformRadarChartData(graphData) {
  ...
  // Transform to Recharts format: [{ metric: "Label", dataset1: value1, dataset2: value2, ... }]
...
/**
 * Analytics Radar Chart Component
 * @param {Object} props
...
export default function AnalyticsRadarChart({
  ...
  const chartData = useMemo(() => {
    if (!graphData) return [];
    return transformRadarChartData(graphData);
  }, [graphData]);
  ...
  const handleExport = () => {
    if (onExport) {
      ...
      // Default CSV export
      const headers = ["Metric", ...Object.keys(datasetLabels)];
