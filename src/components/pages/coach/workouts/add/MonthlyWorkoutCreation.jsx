...
import { changeWorkoutPlans, customWorkoutUpdateField, monthlyWorkoutRP, removeWorkoutFromPlans } from "@/config/state-reducers/custom-workout";
...
export default function MonthlyWorkoutCreation() {
  const [loading, setLoading] = useState(false);
  const { dispatch, selectedDate, ...state } = useCurrentStateContext();
  async function saveCustomWorkout() {
    try {
      setLoading(true);
      const formData = monthlyWorkoutRP({ ...state });
      const response = await sendData(`app/workout/workout-plan/custom`, formData);
  ...
  return <div className="max-w-[450px] mx-auto flex flex-col gap-y-4">
    ...
    <WorkoutMetaData />
    <SelectWorkouts />
    <Button disabled={loading} onClick={saveCustomWorkout}>Save</Button>
  </div>
}
